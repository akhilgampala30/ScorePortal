/**
 * Created by Jacky on 1/1/14.
 */
$(function () {
    var availableTags = new Array(),//list of tags (class and assignment names) to be searched
        matchingTags = new Array(),// accompanying list of the class that each assignment/class belongs to, available and matching tags are same for classes
        classIDList = new Array(); // used to get class ids for redirect
    
    //stolen from http://stackoverflow.com/a/15117411
    ...
    $.post('/include/PHPFunctions/SearchFunctions.php', {opt: 0}).always(function (data) {
        $.each($.parseJSON(data), function (index, cat) {
            classIDList[parseInt(index)] = cat;
        });
    });

    //Get list of autocomplete selections
    $.post('/include/PHPFunctions/SearchFunctions.php', {opt: 1}).always(function (data) {
        $.each($.parseJSON(data), function (name, cat) {
            availableTags.push(name);
            matchingTags.push(cat);
        });
    });

    //Autocomplete code
    $("#SearchBarInput").autocomplete({
    	//see http://jqueryui.com/autocomplete/#folding
        ...
        select: function (event, ui) {
            var selection = ui.item.value,
                cat = matchingTags[availableTags.indexOf(selection)],
                idClass = classIDList.indexOf(cat),
                wrapper = $('#wrapper');

            //redirect code borrowed from leftbarnav

            //change sidebar
            var matchingHref = '/class/' + idClass;
            var matchingTab = $('a[href="' + matchingHref + '"]');
            var DestinationLink = "/class/" + idClass;
            var LinkBits = /\/(class)\/(\d+)/.exec(DestinationLink);
            history.pushState([], '', DestinationLink);
            matchingTab.closest('.class').addClass('selectedclass').siblings().removeClass('selectedclass');

            //change page
            wrapper.animate({'opacity': '0.3'}, 747);
            wrapper.load('/Class/a/1/' + idClass, function (response, status, xhr) {
                wrapper.animate({'opacity': '1'}, 747);
                if (selection != cat)//scroll and expand for assignments, only assignments will have different matching tags
                {
                    var container = $('html, body');
                    var scrollTo = $('div[title="' + selection + '"]');
                    container.animate({
